...
const rclcpp_lifecycle::State & Actuator::configure()
{
  if (impl_->get_state().id() == lifecycle_msgs::msg::State::PRIMARY_STATE_UNCONFIGURED)
  {
    ...
    //the on_configure function is pure virtual , where you need to write it!
    {
      case CallbackReturn::SUCCESS:
        impl_->set_state(
          rclcpp_lifecycle::State(
...
const rclcpp_lifecycle::State & Actuator::deactivate()
{
  ...
  {
    switch (impl_->on_deactivate(impl_->get_state()))//your driver should provide(on_deactivate method) even thought it is not declared in the base class.
    {
      case CallbackReturn::SUCCESS:
        impl_->set_state(
          rclcpp_lifecycle::State(
...
std::vector<StateInterface> Actuator::export_state_interfaces()
{
  ...
  return impl_->export_state_interfaces();//virtual function you must write it yourself.
}
...
return_type Actuator::read(const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  //result is error (serious) only when you make a bad call of read function
