...
public:
  ...
  {
  }// here we initialize the state(or lifecycle_state_) with the initializer list
   ...
   */
  ActuatorInterface(const ActuatorInterface & other) = delete;//know allow copy to exist, anyone who try to call it will get a run time error

  ActuatorInterface(ActuatorInterface && other) = default;//as for other object, their resources can be transferred rather than copied

  virtual ~ActuatorInterface() = default;//their resources can be transferred rather than copied
