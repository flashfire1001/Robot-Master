...
public:
   ...
   */
  CONTROLLER_INTERFACE_PUBLIC
  virtual std::vector<hardware_interface::CommandInterface> export_reference_interfaces() = 0; // you export this not for the hardware but for other controllers.
...
private:
  std::shared_ptr<rclcpp_lifecycle::LifecycleNode> node_;// "it's kind of a plug-in node, shares the same process with controller manager but can be viewed as a new node"
};
